 ...
 *
 * @param {string} reference - путь до страницы описание товара
 ...
 * @param {string} countLike - количество комментариев
 ...
 * @param {Function} click - callback функция для покупки
 * @param {string} as
...
const Card = ({
	...
	return (
		<CustomLink className={classNames(classes.card, className)} href={reference} as={as}>
			<div className={classes.card__img_wrapper}>
				<div className={classes.card__img}>
					...
					<div className={classes.card__status}>
						...
						{choice ? (
							<span className={classes.card__choice}>Выбор покупателей</span>
						) : (
							''
						)}
			...
			<div className={classes.card__footer}>
				<div className={classes.card__price_inner}>
					<div className={classes.card__price}>
						{price} руб.{' '}
						{sale ? (
							<span className={classes.card__price_through}>{price} руб.</span>
						) : (
							''
						)}
					</div>
					<div className={classes.card__weight}>1 000 руб х {weight} кг</div>
				</div>
			...
			{favorites ? (
				<button className={classes.card__favorites_del}>Удалить из избранного</button>
			) : (
				<button className={classes.card__favorites}>Добавить в избранное</button>
			)}
		</CustomLink>
	);
};
